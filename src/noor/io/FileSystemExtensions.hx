...
{
  public static function copyDirRecursively(fs:Class<FileSystem>, srcPath:String, destPath:String) : Void
  {
    ...
    if (FileSystem.exists(srcPath) && FileSystem.isDirectory(srcPath))
    {
      ...
      for (entry in entries)
      {
        if (FileSystem.isDirectory('${srcPath}/${entry}'))
        {
          ...
          FileSystem.copyDirRecursively('${srcPath}/${entry}', '${destPath}/${entry}');
        }
        else
        {
  ...
  public static function deleteDirRecursively(fs:Class<FileSystem>, path:String) : Void
  {
    ...
    {
        FileSystem.deleteDirRecursively('${path}/${entry}');
    }
    else
    {
  ...
  public static function ensureDirExists(fs:Class<FileSystem>, path:String) : Void
  {
